...
class Test:
	...
	def test_deposit(self):
		...
		assert self.p1.get_balance()  == 500
		assert self.p1.deposit(388.8) is True
		assert self.p1.get_balance()  == pytest.approx(888.8, abs=0.001)

		assert self.p2.deposit(1000) is True
		assert self.p2.get_balance()  == 1000
		assert self.p2.deposit(-500) is False
		assert self.p2.get_balance()  == 1000
		assert self.p2.deposit(0) is False
		assert self.p2.get_balance()  == 1000
		assert self.p2.deposit(555.5) is True
		assert self.p2.get_balance()  == pytest.approx(1555.5, abs=0.001)

	def test_withdraw(self):

		assert self.p1.withdraw(500) is False
		assert self.p1.get_balance()  == 0
		assert self.p1.withdraw(-50) is False
		assert self.p1.get_balance()  == 0
		assert self.p1.withdraw(0) is False
		assert self.p1.get_balance()  == 0

		self.p1.deposit(500) 
		assert self.p1.withdraw(20.5) is True
		assert self.p1.get_balance() == pytest.approx(479.5, abs=0.001)

		assert self.p2.withdraw(250) is False
		assert self.p2.get_balance()  == 0
		assert self.p2.withdraw(-5) is False
		assert self.p2.get_balance()  == 0
		assert self.p2.withdraw(0) is False
		assert self.p2.get_balance()  == 0

		self.p2.deposit(1700) 
		assert self.p2.withdraw(500.25) is True
